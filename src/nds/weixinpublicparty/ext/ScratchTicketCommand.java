...
public class ScratchTicketCommand extends Command {
	private static Logger logger= LoggerManager.getInstance().getLogger(ScratchTicketCommand.class.getName());
	...
	public ValueHolder execute(DefaultWebEvent event) throws NDSException,
		...
		String wx_scratchticket_note_id="";
		String username ="";
		String phonenum = "";
		String ticketno = "";
		boolean isErp=false;
		try {
			messagejo=new JSONObject(messagejo.optString("params"));//取值
			logger.debug("WX_SCRATCHTICKET_NOTE params--》"+messagejo);
			wx_scratchticket_note_id = messagejo.getString("wxScratchticketNoteId"); 
		...
		//需要的参数
		String scratchsql = "select S.WX_VIP_ID,S.AD_CLIENT_ID,S.RECORD_STATE,s.WX_SCRATCHREWARD_ID FROM WX_SCRATCHTICKET_NOTE S WHERE S.id = ?";
		JSONObject rejson = QueryEngine.getInstance().doQueryObject(scratchsql.toString(),new Object[] {wx_scratchticket_note_id}); 
		String wx_scratchreward_id = rejson.optString("WX_SCRATCHREWARD_ID");
		...
		String scsql = "select sc.REWARDTYPE,sc.NAME,sc.WX_COUPON_ID,sc.INTEGRAL from WX_SCRATCHREWARD sc where sc.id=?";
		JSONObject awjson = QueryEngine.getInstance().doQueryObject(scsql.toString(),new Object[]{wx_scratchreward_id});
		String reward_type = awjson.optString("REWARDTYPE");
		//String name = awjson.optString("NAME"); 实物名称
		String wx_coupon_id = awjson.optString("WX_COUPON_ID");//优惠券
		int integral = awjson.optInt("INTEGRAL");//积分
		
		//判断记录是否存在中奖记录
		if(rejson==null) {
			logger.debug("WX_SCRATCHTICKET_NOTE not found");
			vh.put("code", "-1");
		...
		if(reward_type.equals("INT")){//奖品为积分
			if(isErp){//线下
				...
				params.put("method","adjustIntegral");//调线下存储过程
				try {
				...
				logger.debug("adjustIntegral offline reutrn:"+result);
				JSONObject jo=null;
				try {
			...
			//不管线下是否接通
			 ...
			 try {
			   ...
			   Collection list=QueryEngine.getInstance().executeFunction("wx_coupon_$r_adjust", params2, para);//调线上存储过程
		...
		//更新中奖记录
		String sql = "update WX_SCRATCHTICKET_NOTE b set b.name ='"+username+"' ,b.phonenum='"+phonenum+"' ,b.record_state="+record_state+" where b.id="+wx_scratchticket_note_id+"";
		logger.debug("!!!!!!!!!!!!!!!!!!!!!!!!!!!");
		try {
