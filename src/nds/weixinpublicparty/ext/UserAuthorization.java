...
public class UserAuthorization implements IAuthorization{
	...
	private final static String seachVip="select wv.ID,wv.OPENCARD_STATUS,vi.id,vi.issubscribe,vi.sex,vi.id from wx_vip wv LEFT JOIN wx_vip_inqury vi ON vi.wx_vip_id=wv.id where wv.id=? and wv.AD_CLIENT_ID=? and ROWNUM=1";
