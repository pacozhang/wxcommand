...
public class SendCouponsCommand extends Command{
	...
	public ValueHolder execute(DefaultWebEvent event) throws NDSException,
		...
		try{
			...
			all=QueryEngine.getInstance().doQueryList("select c.name,c.value,c.minimumcharge,ce.begindate,ce.enddate,v.wechatno from wx_couponemploy ce join wx_coupon c on ce.wx_coupon_id=c.id join wx_vip v on v.id=ce.wx_vip_id where ce.id=?",new Object[] {couponsid});
		}catch(Exception e){
			e.printStackTrace();
			vh.put("code", -1);
		...
		try{
			...
			try {
				...
				sendcoupons.put("template_id", "FIw3uttfOKf95iWkeNSQoEYWTBOCbWoQRxNctgJr6lk");
				sendcoupons.put("url", "http://weixin.qq.com/download");
